...
type ManufacturerOrderStatus = 'pending' | 'confirmed' | 'in_production' | 'packed' | 'shipped' | 'delivered' | 'cancelled';
...
type CustomDesignRequest = {
  ...
  created_at: string;
  custom_design_orders?: { id: string; status: ManufacturerOrderStatus; order_number: string; tracking_number: string | null }[];
};
...
const MFR_STATUS_STYLES: Record<ManufacturerOrderStatus, string> = {
...
const MFR_STATUS_LABELS: Record<ManufacturerOrderStatus, string> = {
  pending: 'Awaiting Manufacturer',
  confirmed: 'Confirmed',
    ...
    <div className="rounded-xl border border-[#e4d8c6] bg-[#FBF9F5] overflow-hidden">
      ...
      {expanded && (
        <div className="border-t border-[#e4d8c6] px-4 pb-4 pt-3 space-y-4 bg-white/60">
          ...
          {/* Manufacturer order status (shown when forwarded) */}
          {request.status === 'forwarded' && request.custom_design_orders && request.custom_design_orders.length > 0 && (
            <div className="space-y-1.5 rounded-lg border border-[#e4d8c6] bg-[#fdf9f2] px-3 py-2.5">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Manufacturer Status</p>
              {request.custom_design_orders.map((o) => (
                <div key={o.id} className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold ${MFR_STATUS_STYLES[o.status] ?? 'bg-muted text-muted-foreground'}`}>
                    {MFR_STATUS_LABELS[o.status] ?? o.status}
                  </span>
                  {o.tracking_number && (
                    <span className="text-xs text-muted-foreground">Tracking: {o.tracking_number}</span>
                  )}
                </div>
              ))}
            </div>
