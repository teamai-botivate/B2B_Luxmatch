...
The 12 demo products now have primary `product_images` rows pointing at real Cloudinary assets under `luxematch/<SHOP_JEWELLER_ID>/products/` in cloud `dyrc4bo4m`. They were copied from the existing `jewellery_search/*` assets with `pnpm import:cloudinary-product-images -- --source-prefix=jewellery_search --limit=12 --apply --replace-seed`. The source assets had hex public IDs and no product metadata, so the import mapped them sequentially to the current 12 products; use explicit product uploads or a mapping pass for exact catalog photography. Do not run `pnpm reindex` unless product image rows or Cloudinary URLs change. Try-on/AR asset work is deferred; do not touch `/try-on`, `product_tryon_assets`, or `showcase-ar-assets` for the current product-photo pass.
...
6. **Product image mapping quality** — real Cloudinary images are now imported and displayed, but the `jewellery_search/*` source assets had no product metadata, so the 12 demo products were mapped sequentially. For accurate catalog photography, upload per-product photos through the jeweller product flow or rerun the importer after adding an explicit mapping layer. Do not touch try-on/AR assets yet.
...
| Storefront images | Real `product_images` wired across all surfaces (`/api/products/by-ids`, fallbacks); 12 Cloudinary product photos imported; try-on untouched | ✅ |
